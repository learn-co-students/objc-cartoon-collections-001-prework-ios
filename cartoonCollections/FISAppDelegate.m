...
-(NSArray *)arrayOfPlaneteerShoutsFromArray:(NSArray *)powers{
    ...
    for(NSString *power in powers){
        ...
        NSString *powerAsShout = [upperCasePower stringByAppendingString:mark];
...
-(NSString *)summonCaptainPlanetWithPowers:(NSArray *)powers{
    ...
    NSMutableString *captain = [[NSMutableString alloc] initWithString:@"Let our powers combine:\n"];
    
    NSArray *powersAsShouts = [self arrayOfPlaneteerShoutsFromArray:powers];
    
    for(NSString *shout in powersAsShouts){
        
        [captain appendFormat:@"%@\n", shout];
        
    }
    [captain appendString:@"Go Planet!"];
    
    return [NSString stringWithString:captain];
